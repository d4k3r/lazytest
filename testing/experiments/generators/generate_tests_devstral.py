...
OUT_DIR = os.path.expanduser("~/clown/3rd_year_project/testing/generated_tests/devstral/run_3")
METRICS_CSV = os.path.join(OUT_DIR, "metrics_devstral_TheAlgorithms.csv")

##### LLM CONFIGURATION #####
API_BASE_URL = "http://localhost:8000/v1"
MODEL_NAME = "cyankiwi/Devstral-Small-2-24B-Instruct-2512-AWQ-4bit"
    ...
    while attempts < MAX_RETRIES:
        ...
        try:
            response = client.chat.completions.create(
                ...
                messages=[
                    {"role": "user",
                     "content": f"You are a Python QA engineer. Output only code. No reasoning, no explanation.\n\n{current_prompt}"}
                ],
                frequency_penalty=0.05,  # Мягкий штраф за зацикливание
                presence_penalty=0.0,
                temperature = 0.15 + max(0, attempts - 1) * 0.025,
                max_tokens=16384,
                timeout=1200,
            )

...
# [https://github.com/d4k3r/lazytest](https://github.com/d4k3r/lazytest)
# ==============================================================================
...
OUT_DIR = os.path.expanduser("~/clown/3rd_year_project/testing/generated_tests/qwen_3.5/run_3")
METRICS_CSV = os.path.join(OUT_DIR, "metrics_qwen_3.5_TheAlgorithms.csv")
...
def process_file(args):
    ...
    - If the algorithm can produce multiple valid outputs (e.g., permutations), assert properties of the result, not an exact hardcoded value.
    - Mocking: Use `patch` for I/O or network calls. Ensure mocked return values have all required attributes explicitly set (e.g., `.text`, `.status_code`).
    ...
    while attempts < MAX_RETRIES:
        ...
        try:
            response = client.chat.completions.create(
                ...
                messages=[
                    {"role": "system",
                     "content": "You are a Python QA engineer. Output only code. No reasoning, no explanation. /no_think"},
                    {"role": "user", "content": current_prompt}
                ],
                # frequency_penalty=0.3,  # Мягкий штраф за зацикливание
                # presence_penalty=0.1,
                temperature=0.2 if attempts == 1 else 0.2 + (attempts * 0.1),
                max_tokens=16384,
                timeout=1200,
                extra_body = {
                    "chat_template_kwargs": {"enable_thinking": False}  # Попытка вырубить рассуждения на уровне vLLM
                }
            )
        ...
        if run_res is not None:
            ...
            - TimeoutError → your test input is too large. Reduce to 5 elements max.

...
OUT_DIR = str(PROJECT_ROOT / "testing" / "generated_tests" / "qwen_3.5" / "run_9_no_repeat_mut_fixed_long_trace")
...
def process_file(args):
    ...
    base_prompt = f"""Output a single ```python``` code block containing pytest unit tests for the code below.
...
- METRIC TARGETS: Maximize branch and line coverage, and prioritize a high Mutation Score.
- BEYOND COVERAGE: Write "strong" tests that detect logic changes (mutants). Do not just call functions; check outputs strictly.
- BOUNDARY & EDGE CASES: Explicitly test None, empty containers, zero, negative numbers, and extreme values.
- ASSERTIONS: Use specific and strict assertions. Assert not only return values but also side effects.
...
- KEEP TEST DATA CONCISE: Use 5-7 elements max. DO NOT generate exhaustive loops.
- DO NOT test code inside "if __name__ == '__main__':" blocks. NEVER test main().
- If testing functions with non-deterministic or unordered output (sets, coordinates), use set() or sorted() in assertions.
    ...
    while attempts < MAX_RETRIES:
        ...
        try:
            ...
            if run_res.returncode == 0:
                ...
                error_output = (run_res.stdout + run_res.stderr).strip()[-15000:]
        ...
        if run_res is not None:
            ...
            if "Failed to import" in error_output or "ModuleNotFoundError" in error_output:
                notes = f"Import Error: {extracted_error}"
            else:
...
STRICT REQUIREMENTS:
- DO NOT decrease test strength or remove boundary cases to fix the error.
- If a Mutation-killing test failed, correct the expected value or mock, but keep the test case.
- Focus on the root cause in the traceback.
- Maintain high branch coverage without sacrificing the strength of your assertions.
